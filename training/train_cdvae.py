...
parser.add_argument(
    "--model_name", default="cdvae", type=str, help="Model name, e.g. sgcif"
)
parser.add_argument(
...
def train(args, attack_fun, model, optimizer, trainloader):
    ...
    for batch_idx, (x, y, _) in tqdm(enumerate(trainloader), total=len(trainloader), desc="Training"):
        ...
        if args.at_mode == "at":
            loss_ce, out = utils.advt(model, x_adv, y)
        elif args.at_mode == "tr":
            loss_ce, out = utils.trades(model, x, x_adv, y)
        else:
            raise KeyError("there is no such adversarial training mode")
        out, hi, xi, mu, logvar = out
        l_mse = F.mse_loss(xi, x)
        loss_kl = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
        loss_kl /= x.size(0) * 3 * args.dim
        loss = 1.0 * l_mse + args.ce * loss_ce + args.kl * loss_kl
        loss.backward()
        optimizer.step()
